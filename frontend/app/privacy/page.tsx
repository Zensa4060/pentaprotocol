      ...
      <div style={{ minHeight: "100vh", background: "#0a0a0f", color: "#e8e8e8", fontFamily: "'Inter', 'Segoe UI', sans-serif" }}>
        ...
        <div style={{ maxWidth: 820, margin: "0 auto", padding: "56px 24px 96px" }}>
          ...
          <Section title="2. Information We Collect">
            ...
            <br /><br />
            <strong style={{ color: "#e8e8e8" }}>Authentication factors:</strong> If you enable two-factor authentication (2FA), we store a server-side TOTP secret bound to your account; the secret itself is never displayed after initial enrolment. Trusted-device tokens (short random strings stored in an HttpOnly cookie named <code style={{ color: "#e8e8e8" }}>pp_device_token</code>) let you skip the 2FA prompt for 30 days on devices you mark as trusted. Because the cookie is HttpOnly, it cannot be read by JavaScript running in the page, and we persist only a SHA-256 hash of each token on the server so a database leak would not yield usable tokens.
            <br /><br />
