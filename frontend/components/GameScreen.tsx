...
const MatchupOverlay = ({ matchupData, showMatchupOverlay, playerSlot, p1Name, user, themeId, t, isRankedGame, matchupCountdown, loadCustomTheme }: MatchupOverlayProps) => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 10000, background: themeId === "space" ? "url(/bg-earth.png) center/cover no-repeat" : t.bg, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <style>{`
...
export default function GameScreen({ themeId, setThemeIdAction, isSingleplayer, gameMode = "singleplayer", difficulty = "medium", setScreenAction, roomCode, playerSlot, playHoverAction, playPlaceAction, playVictoryAction, playDefeatAction, playRulebreakerAction, playTransitionAction, playClickAction, p1Name, matchupData }: Props) {
  ...
  if (showSplash) return (
    <div style={{ position: "fixed", top: 64, left: 0, right: 0, bottom: 0, zIndex: 2, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: themeId === "space" ? "url(/bg-earth.png) center/cover no-repeat" : t.bg, gap: 32, userSelect: "none" }}>
  ...
  if (isMobile) {
    return (
      <div style={{ position: "fixed", top: 52, left: 0, right: 0, bottom: 0, zIndex: 2, background: themeId === "space" ? "url(/bg-earth.png) center/cover no-repeat" : t.bg, overflow: "hidden", userSelect: "none", WebkitUserSelect: "none" }}>
  ...
  // ── DESKTOP LAYOUT ────────────────────────────────────────────────────────
  return (
    <div style={{ position: "fixed", top: 64, left: 0, right: 0, bottom: 0, zIndex: 2, display: "flex", flexDirection: "row", background: themeId === "space" ? "url(/bg-earth.png) center/cover no-repeat" : t.bg, overflow: "hidden", userSelect: "none", WebkitUserSelect: "none" }}>
