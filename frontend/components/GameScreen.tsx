  ...
  return (
    <div style={{ position:"absolute", inset:0, pointerEvents:"none", zIndex:0, borderRadius:"inherit", overflow:"hidden" }}>
      {[
        ...
        <div key={i} style={{
          position:"absolute", borderRadius:"50%", filter:"blur(35px)",
          width:b.w, height:b.h, background:b.color, top:b.top, left:b.left,
          animation:`heatDrift${i} ${12+i*4}s ease-in-out infinite alternate`,
        }}/>
      ))}
    </div>
  );
...
// P1 piece on red board — animated flame SVG
function Flame({ size, cssSize }: { size?: number; cssSize?: string }) {
  const fw = cssSize ?? `${(size ?? 80) * 0.52}px`;
  return (
    <svg width={fw} height={fw} viewBox="0 0 40 40"
      style={{ position:"absolute", zIndex:4, filter:"drop-shadow(0 0 5px rgba(255,80,0,0.9)) drop-shadow(0 0 12px rgba(255,40,0,0.5))" }}>
      <path d="M20,36 Q8,28 10,18 Q12,12 16,10 Q14,16 18,18 Q16,10 22,4 Q24,14 28,16 Q34,18 30,28 Q28,34 20,36 Z"
        fill="#ff4400" opacity="0">
        <animate attributeName="opacity" from="0" to="1" dur="0.15s" fill="freeze"/>
      </path>
      <path d="M20,34 Q12,28 14,20 Q16,16 19,16 Q17,20 20,22 Q22,16 25,18 Q29,22 26,28 Q24,32 20,34 Z"
        fill="#ff8800" opacity="0">
        <animate attributeName="opacity" from="0" to="1" dur="0.15s" begin="0.05s" fill="freeze"/>
      </path>
      <path d="M20,32 Q16,28 17,23 Q19,20 20,21 Q21,20 23,23 Q24,28 20,32 Z"
...
// P2 piece on red board — animated skull SVG
function Skull({ size, cssSize }: { size?: number; cssSize?: string }) {
  const sw = cssSize ?? `${(size ?? 80) * 0.52}px`;
  return (
    <svg width={sw} height={sw} viewBox="0 0 40 40"
      style={{ position:"absolute", zIndex:4, filter:"drop-shadow(0 0 5px rgba(200,0,0,0.9)) drop-shadow(0 0 10px rgba(180,0,0,0.5))" }}>
      <path d="M8,24 Q8,8 20,8 Q32,8 32,24 L32,28 L8,28 Z"
        fill="none" stroke="#cc0000" strokeWidth="2.2"
        strokeDasharray="70" strokeDashoffset="70">
        <animate attributeName="stroke-dashoffset" from="70" to="0" dur="0.25s" fill="freeze"/>
      </path>
      <path d="M11,28 L11,34 L16,34 L16,30 L20,30 L20,34 L24,34 L24,30 L29,30 L29,34 L29,28"
        fill="none" stroke="#cc0000" strokeWidth="2.2"
  ...
  return (
    <div style={{ position:"absolute", inset:0, pointerEvents:"none", zIndex:0, borderRadius:"inherit", overflow:"hidden" }}>
      {[
        ...
        <div key={i} style={{
          position:"absolute", borderRadius:"50%", filter:"blur(28px)",
          width:b.w, height:b.h, background:b.c, top:b.top, left:b.left,
          animation:`iceD${i} ${14+i*3}s ease-in-out infinite alternate`,
        }}/>
      ))}
    </div>
  );
...
// P1 piece on ice board — animated snowflake SVG
function SnowflakePiece({ size, cssSize }: { size?: number; cssSize?: string }) {
  const fw = cssSize ?? `${(size ?? 80) * 0.52}px`;
  return (
    <svg width={fw} height={fw} viewBox="0 0 40 40"
      style={{ position:"absolute", zIndex:4, filter:"drop-shadow(0 0 5px rgba(200,240,255,0.9)) drop-shadow(0 0 10px rgba(160,210,255,0.5))" }}>
      {[0, 60, 120].map(a => (
        <g key={a} transform={`rotate(${a} 20 20)`}>
          <line x1="20" y1="4" x2="20" y2="36" stroke="#c8eeff" strokeWidth="2" strokeLinecap="round"
...
// P2 piece on ice board — animated ice shard SVG
function IceShardPiece({ size, cssSize }: { size?: number; cssSize?: string }) {
  const sw = cssSize ?? `${(size ?? 80) * 0.52}px`;
  return (
    <svg width={sw} height={sw} viewBox="0 0 40 40"
      style={{ position:"absolute", zIndex:4, filter:"drop-shadow(0 0 5px rgba(100,200,255,0.9)) drop-shadow(0 0 10px rgba(60,160,255,0.5))" }}>
      <polygon points="20,4 28,16 36,30 20,36 4,30 12,16"
        fill="rgba(100,200,255,0.15)" stroke="#64c8ff" strokeWidth="2.2"
...
// Full red-board cell with per-cell embers, heat overlay, Flame/Skull or custom piece
function RedCell({
  cellSize, player, isWinCell, isHov, canPlay, blk,
  useFlameSkull, pieceSymbols, p1c, p2c, fontDisplay,
  onClick, onMouseEnter, onMouseLeave,
}: {
  cellSize: string; player: string | null; isWinCell: boolean; isHov: boolean;
  canPlay: boolean; blk: boolean;
  useFlameSkull: boolean;
  pieceSymbols: { p1: string; p2: string };
  p1c: string; p2c: string;
  fontDisplay: string;
  ...
  return (
    <div ref={ref} onClick={onClick} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}
      style={{
        ...
        position: "relative", cursor: canPlay ? (isHov ? "grabbing" : "grab") : "default",
        overflow: "hidden",
        background: blk ? "rgba(180,0,0,0.18)"
          : isWinCell ? (isP1 ? "rgba(70,5,0,0.97)" : "rgba(50,0,0,0.97)")
          : isHov && !player ? "rgba(35,7,3,0.97)" : "rgba(14,3,1,0.97)",
        border: blk ? "2px solid #AA0000"
          : isWinCell ? `1.5px solid ${isP1 ? "rgba(255,80,0,0.7)" : "rgba(200,0,0,0.7)"}`
          : isHov && !player ? "1.5px solid rgba(220,50,0,0.45)" : "1.5px solid rgba(150,20,0,0.35)",
        boxShadow: isWinCell ? `0 0 18px ${isP1 ? "rgba(255,80,0,0.5)" : "rgba(200,0,0,0.5)"}`
          : isHov && !player ? "0 0 10px rgba(220,40,0,0.15)" : "none",
      ...
      {player === "P1" && (
        useFlameSkull
          ? <Flame size={numSize}/>
          : <span style={{ fontFamily:fontDisplay, fontSize:"clamp(24px,5.5vmin,58px)", fontWeight:700, color:ec, textShadow:`0 0 14px ${ec}88`, position:"relative", zIndex:4 }}>{pieceSymbols.p1}</span>
      )}
      {player === "P2" && (
        useFlameSkull
          ? <Skull size={numSize}/>
          : <span style={{ fontFamily:fontDisplay, fontSize:"clamp(24px,5.5vmin,58px)", fontWeight:700, color:ec, textShadow:`0 0 14px ${ec}88`, position:"relative", zIndex:4 }}>{pieceSymbols.p2}</span>
      )}
      {!player && blk && <span style={{ fontSize:"clamp(14px,2.5vmin,28px)", color:"#AA0000", position:"relative", zIndex:5 }}>✕</span>}
    </div>
  );
}
// Full ice-board cell with frost crystals, ice overlay, Snowflake/IceShard or custom piece
function IceCell({
  cellSize, player, isWinCell, isHov, canPlay, blk,
  useSnowflakeShard, pieceSymbols, p1c, p2c, fontDisplay,
  onClick, onMouseEnter, onMouseLeave,
}: {
  cellSize: string; player: string | null; isWinCell: boolean; isHov: boolean;
  canPlay: boolean; blk: boolean;
  useSnowflakeShard: boolean;
  pieceSymbols: { p1: string; p2: string };
  p1c: string; p2c: string;
  fontDisplay: string;
  onClick: () => void; onMouseEnter: () => void; onMouseLeave: () => void;
    ...
    <div ref={ref} onClick={onClick} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}
      style={{
        ...
        background: blk
          ...
          : isWinCell
            ? (isP1
              ? "linear-gradient(135deg,rgba(5,25,45,0.97),rgba(2,15,30,0.99))"
              : "linear-gradient(135deg,rgba(8,20,40,0.97),rgba(4,12,28,0.99))")
            : isHov && !player
              ? "linear-gradient(135deg,rgba(8,18,35,0.96),rgba(4,10,22,0.98))"
              : "linear-gradient(135deg,rgba(5,12,25,0.96),rgba(2,7,16,0.98))",
        border: blk
          ? "2px solid #0066BB"
          : isWinCell
            ? `1.5px solid ${isP1 ? "rgba(200,240,255,0.65)" : "rgba(100,200,255,0.65)"}`
            : isHov && !player
              ? "1.5px solid rgba(160,220,255,0.45)"
              : "1.5px solid rgba(80,160,220,0.35)",
        boxShadow: isWinCell
          ? `0 0 18px ${isP1 ? "rgba(200,240,255,0.4)" : "rgba(100,200,255,0.4)"}`
          : isHov && !player ? "0 0 10px rgba(140,210,255,0.15)" : "none",
        transition: "background 0.15s, border 0.15s, box-shadow 0.15s",
        flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center",
        opacity: blk ? 0.45 : 1,
      ...
      {player === "P1" && (
        useSnowflakeShard
          ? <SnowflakePiece size={numSize} />
          : <span style={{ fontFamily:fontDisplay, fontSize:"clamp(24px,5.5vmin,58px)", fontWeight:700, color:ec, textShadow:`0 0 14px ${ec}88`, position:"relative", zIndex:4 }}>{pieceSymbols.p1}</span>
      )}
      {player === "P2" && (
        useSnowflakeShard
          ? <IceShardPiece size={numSize} />
          : <span style={{ fontFamily:fontDisplay, fontSize:"clamp(24px,5.5vmin,58px)", fontWeight:700, color:ec, textShadow:`0 0 14px ${ec}88`, position:"relative", zIndex:4 }}>{pieceSymbols.p2}</span>
      )}
...
// Coin face component using real coin images
function CoinFace({ type, size = 82 }: { type: "PENTA" | "PROTO"; size?: number }) {
  const isPenta = type === "PENTA";
  return (
    <div style={{
      width: size, height: size, borderRadius: "50%", overflow: "hidden", flexShrink: 0,
      background: isPenta ? "#ffffff" : "#0a0a0a",
      boxShadow: isPenta
        ? "inset 0 0 12px rgba(0,0,0,0.15)"
        : "inset 0 0 12px rgba(255,200,50,0.1)",
    }}>
      <img
        src={isPenta ? "/penta-coin.png" : "/proto-coin.png"}
        alt={type}
        width={size}
        height={size}
...
// TossCard must live OUTSIDE GameScreen so React doesn't recreate the component
// type on every render (which would break useState and click handlers).
// React.memo prevents re-renders when parent's RAF loop fires at 60fps.
  ...
  return (
    <button
      ...
      style={{
        ...
        border: `2px solid ${hov ? actorCol : borderCol}`,
        borderRadius: ip ? 2 : 16,
        padding: "28px 20px",
        cursor: "pointer",
        display: "flex", alignItems: "center", justifyContent: "center",
        fontFamily: fontDisplay, fontSize: ip ? 16 : 22, fontWeight: 700,
        color: hov ? actorCol : textCol,
        ...
        transition: [
          ...
          "color 0.28s cubic-bezier(.22,.68,0,1.2)",
...
export default function GameScreen({ themeId, setThemeId, isSingleplayer, gameMode = "singleplayer", difficulty = "medium", setScreen, playHover, playPlace, playVictory, playDefeat, playRulebreaker, playTransition, playClick }: Props) {
  ...
  // ── Custom theme (reactive — syncs on storage + focus) ──
  ...
  // isRedBoard: board atmosphere (dark bg, embers, heat, red labels) — triggers on board skin alone
  const isRedBoard     = boardSkin === "red_grid";
  // useFlameSkull: render Flame/Skull SVG pieces whenever flame_skull skin is equipped
  const useFlameSkull  = pieceSkin === "flame_skull";
  // ADD these two lines alongside isRedBoard / useFlameSkull
...
const useSnowflakeShard = pieceSkin === "snowflake_shard";
  ...
  // p1c/p2c: use piece skin colors when a non-default skin is active.
  // Exception: on red_grid board with default piece skin, P2 gets yellow so it's visible
  // against the near-black background (theme's t.p2 is often white/light).
  ...
  const pausedRef = useRef(false); // freeze RAF while modal is open
  ...
  const isAiMode = gameMode === "ai" || (gameMode === "singleplayer" && difficulty !== undefined && isSingleplayer !== true);
  ...
  const R = useRef({
    ...
    current: "P1", winner: null as string|null,
    p1Ready: false, p2Ready: false,
    readyTimeout: 60, readyTimer: 0,
    coinResult: null as "PENTA"|"PROTO"|null,
    matchOver: false, gameNumber: 1,
    matchHistory: [] as string[],
    firstPlayerChosen: null as string|null,
    tossWinner: null as "P1"|"P2"|null,
    rbC3Blocked: false, summaryTimer: 3.0,
    choiceTimer: 0,
  });
  ...
  useEffect(() => {
    ...
    // Delays per difficulty to feel natural
  ...
  useEffect(() => {
    ...
    if (dur !== undefined) {
      choiceTimerRef.current = dur;
      lastChoiceSec.current = dur;
      setChoiceTimer(dur);
    }
  }, [phase]);
  ...
  const choiceTimerRef = useRef(0);   // high-freq ref — avoids 60fps re-renders
  const lastChoiceSec  = useRef(-1);  // last integer second we rendered
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      // Once winner is set, stop the game clock — but keep
      // waiting_ready and rb phase timers running
      if (s.winner && !freePhases.includes(s.phase)) return;

      // Freeze all other state updates while a modal is open (but not during free phases)
      if (pausedRef.current && !freePhases.includes(s.phase)) return;

      if (s.phase === "playing" && !s.winner) {
        if (s.current === "P1")
          setP1Time(v => { if (v - dt <= 0) { setWinner("P2"); return 0; } return v - dt; });
        else
          setP2Time(v => { if (v - dt <= 0) { setWinner("P1"); return 0; } return v - dt; });
      }
      ...
      if (s.phase === "waiting_ready") {
        if (!s.p1Ready || !s.p2Ready) {
          setReadyTimeout(v => {
            const nv = v - dt / 1000;
            if (nv <= 0) { setP1Ready(true); setP2Ready(true); setReadyTimer(1); return 0; }
            return nv;
          });
        } else if (s.readyTimer > 0) {
          setReadyTimer(v => {
            const nv = v - dt / 1000;
            if (nv <= 0) { doAdvanceAfterReady(); return 0; }
            return nv;
          });
        }
      }

      if (s.phase === "rb_splash")
        setRbSplashTimer(v => {
          const nv = v - dt/1000;
          if (nv <= 0) { setPhase("rb_coin"); return 3; }
          return nv;
        });
      ...
      if (s.phase === "rb_coin") {
        ...
        if (!s.coinResult) {
          ...
          setCoinRevealTimer(v => {
            const nv = v - dt/1000;
            if (nv <= 0) { setPhase("rule_choice"); return 0; }
            return nv;
          });
        }
      }
      ...
      if (tossChoicePhases.includes(s.phase) && s.choiceTimer > 0) {
        ...
        if (choiceTimerRef.current <= 0) {
          ...
          setChoiceTimer(0);
          autoPickLeft(s.phase);
        } else {
          // Only re-render once per second (for the countdown number + bar)
  ...
  const doAdvanceAfterReady = () => {
    ...
    setOverlayVisible(false);
    if (gn >= 2) {
      // Game 2 just ended → entering game 3 (Rulebreaker)
      setGameNumber(3);
      setPhase("rb_splash"); playRulebreaker?.();
      ...
      // Game 1 just ended → start game 2
      setGameNumber(2);
      setPhase("playing");
      initBoard("P2");
    }
  ...
  useEffect(() => {
    ...
    // Double rAF: first frame mounts overlay at opacity:0, second frame sets opacity:1
    // so the CSS transition actually fires. Single rAF batches both = no transition.
    ...
    if (newHist.length >= 2) {
      ...
      if (sw !== null) {
        setMatchOver(true); setSeriesWinner(sw); setPhase("match_over");
      } else if (gn >= 3) {
        setMatchOver(true); setSeriesWinner(newHist[newHist.length - 1]); setPhase("match_over");
      } else {
        setP1Ready(false); setP2Ready(false); setReadyTimeout(60); setReadyTimer(0);
  ...
  useEffect(() => {
    if (phase === "waiting_ready" && p1Ready && p2Ready && R.current.readyTimer <= 0)
      setReadyTimer(1);
  }, [p1Ready, p2Ready]);

  const place = async (r: number, c: number) => {
    if (phase !== "playing" || board[r][c] || winner || loading) return;
    if (gameMode === "ai" && current === "P2") return; // bot's turn — block human click
  ...
  const dismissOverlay = useCallback(() => {
    setOverlayVisible(false);
    setTimeout(() => setShowWinOverlay(false), 320);
  }, []);
  ...
  if (showSplash) return (
    <div style={{ position:"fixed", top:64, left:0, right:0, bottom:0, zIndex:2, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:t.bg, gap:32, userSelect:"none" }}>
      <div style={{ fontFamily:t.fontDisplay, fontSize:"clamp(24px,5vw,72px)", fontWeight:900, color:t.accent, textShadow:`0 0 60px ${t.accentGlow}55`, letterSpacing:"0.06em", textAlign:"center" }}>
        SINGLEPLAYER
      </div>
      <div style={{ fontFamily:t.fontBody, fontSize:"clamp(13px,1.6vw,18px)", color:t.textSecondary, letterSpacing:"0.04em" }}>
        Local · Pass & Play · Best of 3
      </div>
      <button
      ...
      <div style={{ position:"absolute", inset:0, display:"flex", alignItems:"center", justifyContent:"center", pointerEvents:"none" }}>
        {[1,2,3].map(i => (
          <div key={i} style={{
            ...
            borderRadius:"50%",
            border:`1px solid ${t.accent}${["22","18","0C"][i-1]}`,
            animation:`rbRingPulse 1.8s cubic-bezier(.22,.68,0,1.2) ${i * 0.18}s both`,
          }} />
        ))}
      </div>
      <div style={{ display:"flex", gap: ip ? 2 : 4, alignItems:"center", justifyContent:"center" }}>
        {"RULEBREAKER".split("").map((ch, i) => (
          <span key={i} style={{
            ...
            fontWeight: 900,
            color: t.accent,
            textShadow: `0 0 60px ${t.accentGlow}88`,
            letterSpacing: "0.01em",
            display: "inline-block",
            animation: `rbLetterIn 0.55s cubic-bezier(.22,.68,0,1.2) ${i * 0.045}s both`,
          }}>{ch}</span>
        ))}
      </div>
      <div style={{
        fontFamily: t.fontMono, fontSize: ip ? 11 : 14,
        color: t.textMuted, letterSpacing: "0.28em",
        marginTop: 18,
        animation: "rbSubIn 0.6s cubic-bezier(.22,.68,0,1.2) 0.55s both",
      }}>
        ROUND 3 — SPECIAL RULES APPLY
      </div>
      <div style={{
        width: "clamp(200px,38vw,480px)", height: 2,
        background: `linear-gradient(90deg, transparent, ${t.accent}, transparent)`,
        marginTop: 20,
        animation: "rbLineIn 0.7s cubic-bezier(.22,.68,0,1.2) 0.45s both",
        boxShadow: `0 0 18px ${t.accentGlow}66`,
      }} />
    </div>
  );
    ...
    return (
      ...
      >
        ...
        <div style={{
          ...
          fontWeight: 900,
          color: t.accent,
          textShadow: `0 0 40px ${t.accentGlow}66`,
        ...
        <div style={{
          ...
          background: `linear-gradient(90deg, transparent, ${t.accent}, transparent)`,
          marginBottom: 18,
          boxShadow: `0 0 14px ${t.accentGlow}55`,
          animation: "rbLineIn 0.6s cubic-bezier(.22,.68,0,1.2) 0.1s both",
        }} />

        {/* Legend */}
        <div style={{
          display: "flex", gap: 28,
          fontFamily: t.fontMono, fontSize: 39, color: t.textMuted,
          marginBottom: 20,
          animation: "fadeUp 0.5s cubic-bezier(.22,.68,0,1.2) 0.12s both",
        }}>
          <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <CoinFace type="PENTA" size={26} />
            <span>PENTA = P1</span>
          </span>
          <span style={{ color: t.border }}>|</span>
          <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <CoinFace type="PROTO" size={26} />
            <span>PROTO = P2</span>
          </span>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            borderRadius: "50%",
            background: revealed
              ? `radial-gradient(circle, ${winCol}28 0%, transparent 68%)`
              : `radial-gradient(circle, ${t.accent}14 0%, transparent 68%)`,
            transition: "background 0.6s ease",
            pointerEvents: "none",
          }} />

          {/* Spinning rings */}
          {!revealed && [1, 1.5, 2].map((scale, i) => (
            <div key={i} style={{
              position: "absolute",
              width: coinDiam * scale,
              height: coinDiam * scale,
              borderRadius: "50%",
              border: `1px solid ${t.accent}${["18","10","08"][i]}`,
              animation: `spinRing ${2 + i * 0.4}s linear infinite`,
              pointerEvents: "none",
            }} />
          ))}
          ...
          {revealed ? (
            ...
            <div style={{
              ...
              alignItems: "center",
              gap: 18,
              animation: "coinReveal 0.6s cubic-bezier(.22,.68,0,1.2) both",
            }}>
              <div style={{
                borderRadius: "50%",
                boxShadow: `0 0 90px ${winCol}66, 0 0 40px ${winCol}33, 0 20px 60px rgba(0,0,0,0.7)`,
              }}>
                <CoinFace type={revType} size={coinDiam} />
              </div>
              <span style={{
                fontFamily: t.fontDisplay,
                ...
                textShadow: `0 0 32px ${winCol}99`,
                animation: "fadeUp 0.4s cubic-bezier(.22,.68,0,1.2) 0.18s both",
              }}>
                {revType}
              </span>
            </div>
          ) : (
            /* SPINNING — manual face swap at 90°/270° edge points */
            (() => {
              const deg = ((coinAngle * (180 / Math.PI)) % 360 + 360) % 360;
              // scaleX: cos of angle gives -1..1, abs gives the squash
              const scaleX = Math.cos(coinAngle * 2);
              // Which face: penta 0-90 & 270-360, proto 90-270
              ...
              return (
                <div style={{
                  ...
                  <img src={src} alt={faceIsPenta ? "PENTA" : "PROTO"}
        ...
        {revealed && (
          <div style={{
            ...
            marginTop: 8,
            animation: "fadeUp 0.5s cubic-bezier(.22,.68,0,1.2) 0.3s both",
          }}>
            <span style={{ color: winCol }}>{tossWinner}</span>
            <span style={{ color: t.textMuted }}> WINS THE TOSS</span>
          </div>
        )}
      </div>
    );
  ...
  if (tossChoicePhases.includes(phase)) {
    ...
    return (
      <div className="phase-screen" style={{ position:"fixed", top:0, left:0, right:0, bottom:0, zIndex:2, overflowY:"auto", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:t.bg, padding:"40px 24px", gap:24, userSelect:"none" }}>
        <style>{`
          @keyframes cardSlideIn { from{opacity:0;transform:translateY(22px)} to{opacity:1;transform:translateY(0)} }
          .toss-card-enter { animation: cardSlideIn 0.45s cubic-bezier(.22,.68,0,1.2) both; animation-fill-mode: both; }
        `}</style>
        ...
        <div style={{ width:"min(480px,88vw)", display:"flex", flexDirection:"column", gap:6 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <span style={{ fontFamily:t.fontMono, fontSize:11, color:t.textMuted, letterSpacing:"0.12em" }}>
              {actor} IS CHOOSING
            </span>
            <span style={{ fontFamily:t.fontMono, fontSize:22, fontWeight:700, color: urgent ? t.danger : actorCol, transition:"color 0.3s ease", animation: urgent ? "urgentPulse 0.6s ease infinite" : "none" }}>
              {fmtSec(choiceTimer)}s
            </span>
          </div>
        ...
        <div style={{ display:"flex", gap:20, width:"100%", maxWidth:880 }}>
          <TossCard
            label={leftLabel} onClick={onLeft} delay={0.12}
            actorCol={actorCol} bgCard={t.bgCard} borderCol={t.border}
            textCol={t.text} fontDisplay={t.fontDisplay} ip={ip}
          />
          <TossCard
            label={rightLabel} onClick={onRight} delay={0.20}
            actorCol={actorCol} bgCard={t.bgCard} borderCol={t.border}
            textCol={t.text} fontDisplay={t.fontDisplay} ip={ip}
          />
        </div>
      ...
      <div style={{ position:"fixed", top:0, left:0, right:0, bottom:0, zIndex:2, overflowY:"auto", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:t.bg, padding:"40px 24px", gap:24, userSelect:"none", animation:"fadeUp 0.35s ease both" }}>
        ...
        <div style={{ display:"flex", gap:20, width:"100%", maxWidth:800 }}>
          {(["P1","P2"] as const).map(p => {
            ...
            return (
              <div key={p} style={{ flex:1, background:t.bgCard, border:`3px solid ${col}`, borderRadius:ip?2:14, padding:"24px 20px", textAlign:"center" }}>
                <div style={{ fontFamily:t.fontDisplay, fontSize:50, fontWeight:900, color:col, marginBottom:12 }}>{p}</div>
    ...
    <div style={{ position:"fixed", top:64, left:0, right:0, bottom:0, zIndex:2, display:"flex", flexDirection:"row", background:t.bg, overflow:"hidden", userSelect:"none", WebkitUserSelect:"none" }}>
      ...
      {showWinOverlay && winner && (
        <div onClick={dismissOverlay} style={{ position:"fixed", inset:0, zIndex:999, cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"stretch", willChange:"opacity", opacity:overlayVisible?1:0, transition:"opacity 0.28s ease" }}>
          {/* Dark bg as separate layer — avoids animating background color (causes repaint) */}
          ...
          {seriesDiffers ? (
            <>
              <div style={{ position:"relative", zIndex:1, flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", borderBottom:"1px solid #ffffff14", willChange:"transform, opacity", opacity:overlayVisible?1:0, transform:overlayVisible?"translateY(0)":"translateY(28px)", transition:"opacity 0.32s ease 0.08s, transform 0.32s cubic-bezier(.22,.68,0,1.2) 0.08s" }}>
                ...
                <div style={{ fontFamily:t.fontDisplay, fontSize:"clamp(40px,6.5vw,90px)", fontWeight:900, color:winnerColor, lineHeight:1, textShadow:`0 0 60px ${winnerColor}88`, animation:"winPulse 1.6s ease infinite" }}>
              ...
              <div style={{ position:"relative", zIndex:1, flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", willChange:"transform, opacity", opacity:overlayVisible?1:0, transform:overlayVisible?"translateY(0)":"translateY(28px)", transition:"opacity 0.32s ease 0.18s, transform 0.32s cubic-bezier(.22,.68,0,1.2) 0.18s" }}>
                ...
                <div style={{ fontFamily:t.fontDisplay, fontSize:"clamp(40px,6.5vw,90px)", fontWeight:900, color:seriesColor, lineHeight:1, textShadow:`0 0 60px ${seriesColor}88`, animation:"winPulse 1.6s ease infinite" }}>
                  {seriesWinner} WINS!
                </div>
            ...
            <div style={{ position:"relative", zIndex:1, flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", textAlign:"center", willChange:"transform, opacity", opacity:overlayVisible?1:0, transform:overlayVisible?"translateY(0) scale(1)":"translateY(32px) scale(0.96)", transition:"opacity 0.32s ease 0.06s, transform 0.35s cubic-bezier(.22,.68,0,1.2) 0.06s" }}>
              ...
              <div style={{ fontFamily:t.fontDisplay, fontSize:"clamp(44px,7vw,100px)", fontWeight:900, color:winnerColor, lineHeight:1, marginBottom:18, textShadow:`0 0 60px ${winnerColor}88`, animation:"winPulse 1.6s ease infinite" }}>
                {winner === "DRAW" ? "DRAW" : `${winner} WINS!`}
              </div>
              {phase === "match_over"
                ? <div style={{ fontFamily:t.fontMono, fontSize:"clamp(13px,1.8vw,18px)", color:"#AAAAAA", marginBottom:20 }}>MATCH OVER — SERIES COMPLETE</div>
                : <div style={{ fontFamily:t.fontMono, fontSize:"clamp(13px,1.8vw,18px)", color:"#AAAAAA", marginBottom:20 }}>GAME {R.current.gameNumber} COMPLETE</div>
              }
      ...
      <div style={{ width:panelW, flexShrink:0, background:t.bgPanel, borderRight:`${ip?3:1}px solid ${t.border}`, padding:"18px 18px", display:"flex", flexDirection:"column", gap:14, overflowY:"auto" }}>
        ...
        <div style={{ borderTop:`1px solid ${t.border}`, paddingTop:12 }}>
          ...
          {seriesWinner && (
            <div style={{ marginTop:10, fontFamily:t.fontMono, fontSize:20, color:t.gold, textAlign:"center", fontWeight:700 }}>
              SERIES: {seriesWinner==="DRAW"?"DRAW":seriesWinner+" WINS"}
            </div>
          )}
        </div>
        {phase==="waiting_ready" && (
          <div style={{ display:"flex", flexDirection:"column", gap:8, animation:"fadeUp 0.3s ease both" }}>
            ...
            {(["P1","P2"] as const).map(p => {
              ...
              const col = p==="P1" ? p1c : p2c;
              return (
                <button key={p} onClick={() => p==="P1"?setP1Ready(v=>!v):setP2Ready(v=>!v)}
        ...
        {phase==="match_over" && (
          <div style={{ textAlign:"center", animation:"fadeUp 0.3s ease both" }}>
            <div style={{ fontFamily:t.fontDisplay, fontSize:16, fontWeight:700, color:t.gold, marginBottom:10 }}>
              {seriesWinner==="DRAW"?"DRAW!":seriesWinner+" WINS!"}
            </div>
        ...
        {(phase==="playing"||phase==="waiting_ready") && (
          <div style={{ display:"flex", flexDirection:"column", gap:8, marginTop:"auto", borderTop:`1px solid ${t.border}`, paddingTop:12 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              ...
              <button onClick={() => setChatOpen(v=>!v)} style={{ background:"none", border:"none", color:t.text, fontFamily:t.fontMono, fontSize:16, cursor:"pointer", padding:"2px 6px" }}>
                {chatOpen ? "▾" : "▸"}
              </button>
            </div>
            {chatOpen && (
              <>
                <div style={{ height:160, overflowY:"auto", background:t.bgCard, border:`1px solid ${t.border}`, borderRadius:ip?2:8, padding:"10px 12px", display:"flex", flexDirection:"column", gap:6 }}>
                  {chatMessages.length === 0 && (
                    <div style={{ fontFamily:t.fontBody, fontSize:14, color:t.textMuted, textAlign:"center", marginTop:24 }}>No messages yet</div>
                  )}
                  {chatMessages.map((m,i) => (
                    <div key={i} style={{ display:"flex", gap:6, alignItems:"flex-start" }}>
                      <span style={{ fontFamily:t.fontMono, fontSize:14, fontWeight:700, color:m.from==="P1"?p1c:p2c, flexShrink:0 }}>{m.from}:</span>
                      <span style={{ fontFamily:t.fontBody, fontSize:14, color:t.text, wordBreak:"break-word" as const }}>{m.text}</span>
                    </div>
                  ))}
                </div>
                {chatWarning && (
                  <div style={{ padding:"8px 12px", background:"#F4433618", border:"1px solid #F44336", borderRadius:6, fontFamily:t.fontBody, fontSize:13, color:"#F44336" }}>
                    ⚠ Inappropriate language detected and censored.
                  </div>
                )}
                <div style={{ display:"flex", gap:6 }}>
                  <input
                    ...
                    onKeyDown={e => { if (e.key==="Enter") sendChat("P1"); }}
                    placeholder="message…"
                    maxLength={60}
                    style={{ flex:1, background:t.inputBg, border:`1px solid ${t.border}`, borderRadius:ip?2:6, color:t.text, fontFamily:t.fontBody, fontSize:14, padding:"8px 10px", outline:"none", minWidth:0 }}
                  />
                  <button onClick={() => sendChat("P1")}
                    style={{ background:`${p1c}20`, border:`1px solid ${p1c}`, color:p1c, fontFamily:t.fontMono, fontSize:14, fontWeight:700, padding:"8px 12px", borderRadius:ip?2:6, cursor:"pointer", transition:"all 0.18s", flexShrink:0 }}
                    onMouseEnter={e => { e.currentTarget.style.background=p1c; e.currentTarget.style.color="#000"; }}
                    onMouseLeave={e => { e.currentTarget.style.background=`${p1c}20`; e.currentTarget.style.color=p1c; }}
                  >P1</button>
                  <button onClick={() => sendChat("P2")}
                    style={{ background:`${p2c}20`, border:`1px solid ${p2c}`, color:p2c, fontFamily:t.fontMono, fontSize:14, fontWeight:700, padding:"8px 12px", borderRadius:ip?2:6, cursor:"pointer", transition:"all 0.18s", flexShrink:0 }}
                    onMouseEnter={e => { e.currentTarget.style.background=p2c; e.currentTarget.style.color="#000"; }}
                    onMouseLeave={e => { e.currentTarget.style.background=`${p2c}20`; e.currentTarget.style.color=p2c; }}
                  >P2</button>
                </div>
              </>
            )}
        ...
        {(phase==="playing"||phase==="waiting_ready") && (
          isRankedGame ? (
            <button
              onClick={() => { playClick?.(); pausedRef.current = true; setShowSurrender(true); }}
              ...
              onMouseEnter={e => { playHover?.(); e.currentTarget.style.background=`${t.danger}30`; }}
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:10, padding:"10px 0", minWidth:0 }}>
        ...
        {/* Turn indicator row — fixed height, pill always centred, hint fades out */}
        <div style={{ height: 36, display:"flex", alignItems:"center", justifyContent:"center", gap:16, width:"100%", position:"relative", paddingLeft:"2%" }}>
          {/* Hint LEFT — visible before first move, invisible after (keeps width for centering) */}
          <div style={{
            fontFamily:t.fontMono, fontSize:11, letterSpacing:"0.08em",
            background: c3Blocked ? `${t.danger}10` : `${t.gold}10`,
            border:`1px solid ${c3Blocked ? t.danger : t.gold}33`,
            borderRadius:6, padding:"3px 14px",
            color: c3Blocked ? t.danger : t.gold,
            flexShrink:0,
            visibility: phase==="playing" && movesPlayed===0 ? "visible" : "hidden",
            opacity: phase==="playing" && movesPlayed===0 ? 1 : 0,
          ...
          {/* Turn pill — always centred because hint mirrors on both sides */}
          ...
          {/* Mirror spacer RIGHT — same width as hint, always invisible, keeps pill centred */}
          <div style={{
            fontFamily:t.fontMono, fontSize:11, letterSpacing:"0.08em",
            borderRadius:6, padding:"3px 14px",
            flexShrink:0,
        ...
        <div style={{ display:"flex", gap:6, alignItems:"flex-start" }}>
          ...
          <div style={{ position:"relative", display:"grid", gridTemplateColumns:`repeat(5,${bigCs})`, gridTemplateRows:`repeat(5,${bigCs})`, gap:`${boardGap}px`, background: isRedBoard ? "rgba(10,2,1,0.99)" : isIceBoard ? "linear-gradient(135deg,rgba(3,8,20,0.98),rgba(1,4,14,0.99))" : t.boardLine
, padding:`${boardPad}px`, borderRadius:ip?2:10, border: `${ip?3:2}px solid ${isRedBoard ? "rgba(140,20,0,0.35)" : isIceBoard ? "rgba(80,160,220,0.28)" : t.border}`, boxShadow: isRedBoard ? "0 0 50px rgba(180,20,0,0.1), inset 0 0 40px rgba(0,0,0,0.7)"
         ...
         : "none", overflow:"hidden" }}>
            ...
            {board.map((row, r) => row.map((cell, c) => {
              ...
              // After the RedCell block, before the default cell:
if (isIceBoard) {
  return (
    <IceCell
      key={key}
      ...
      canPlay={canPlay}
      blk={blk}
      useSnowflakeShard={useSnowflakeShard}
      pieceSymbols={pieceSymbols}
      p1c={p1c}
      p2c={p2c}
      fontDisplay={t.fontDisplay}
    ...
    />
              ...
              return (
                <div key={key} onClick={() => place(r,c)} onMouseEnter={() => { setHover(key); }} onMouseLeave={() => setHover(null)}
                  className={isWin ? "win-cell-pulse" : ""}
                  style={{
                    "--win-col": ec,
                    width:bigCs, height:bigCs,
                    background: blk?`${t.danger}18`:isWin?`${ec}28`:isHov?`${cc}22`:t.boardBg,
                    border:`2px solid ${blk?t.danger:isWin?ec:isHov?cc:t.boardLine}`,
                    borderRadius:ip?0:4, display:"flex", alignItems:"center", justifyContent:"center",
                    cursor:canPlay?(isHov?"grabbing":"grab"):"default",
                    fontSize:"clamp(24px,5.5vmin,58px)", fontFamily:t.fontDisplay, fontWeight:700,
                    color:ec,
                    textShadow:isWin?`0 0 20px ${ec}`:cell?`0 0 14px ${ec}77`:"none",
                    transition:"background 0.1s, border-color 0.1s",
                    opacity:blk?0.4:1,
                    boxShadow:isWin?`0 0 8px ${ec}44`:isHov?`inset 0 0 12px ${cc}22`:"none",
                    willChange:isWin?"auto":canPlay?"background, border-color":"auto",
                    position:"relative",
      ...
      <div style={{ width:panelW, flexShrink:0, background:t.bgPanel, borderLeft:`${ip?3:1}px solid ${t.border}`, padding:"18px 18px", display:"flex", flexDirection:"column", gap:10, overflowY:"auto" }}>
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", gap:4 }}>
          {log.length===0
            ? <div style={{ fontFamily:t.fontBody, fontSize:14, color:t.textMuted, fontStyle:"italic" }}>No moves yet</div>
            : log.map((m,i) => <div key={i} style={{ fontFamily:t.fontMono, fontSize:15, color:m.player==="P1"?p1c:p2c, padding:"3px 0", borderBottom:`1px solid ${t.border}22` }}>{m.text}</div>)}
        </div>
        {setScreen && !isRankedGame && (phase==="playing"||phase==="waiting_ready"||phase==="match_over") && (
          <button
            ...
            style={{ background:`${t.danger}16`, border:`1px solid ${t.danger}`, color:t.danger, fontFamily:t.fontBody, fontSize:13, padding:9, borderRadius:ip?2:6, cursor:"pointer", transition:"all 0.2s", marginTop:4 }}
            onMouseEnter={e => { playHover?.(); e.currentTarget.style.background=`${t.danger}30`; }}
            onMouseLeave={e => { e.currentTarget.style.background=`${t.danger}16`; }}
          >✕ EXIT MATCH</button>
        )}
      ...
      {showSurrender && (
        <div className="overlay-backdrop" style={{ position:"fixed", inset:0, zIndex:9999, background:"rgba(0,0,0,0.92)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:28 }}>
          <div className="overlay-modal" style={{ background:t.bgPanel, border:`${ip?3:2}px solid ${t.danger}`, borderRadius:ip?2:20, padding:ip?"32px 36px":"48px 56px", maxWidth:520, width:"90vw", textAlign:"center", boxShadow:`0 40px 100px rgba(0,0,0,0.8), 0 0 60px ${t.danger}22` }}>
            ...
            <div style={{ fontFamily:t.fontDisplay, fontSize:ip?14:23, fontWeight:700, color:t.danger, lineHeight:1.5, marginBottom:12 }}>
              Are you sure you want to forfeit this Match?
            </div>
            <div style={{ fontFamily:t.fontBody, fontSize:ip?11:15, color:t.textMuted, marginBottom:36, lineHeight:1.7 }}>
              {isRankedGame
                ? <>This counts as a <span style={{color:t.danger,fontWeight:700}}>forfeit</span> and will result in <span style={{color:t.danger,fontWeight:700}}>ELO deduction</span>.</>
                : "Your opponent will be declared the winner."}
            </div>
            <div style={{ display:"flex", gap:16, justifyContent:"center" }}>
              <button className="action-btn"
                onClick={() => { setShowSurrender(false); if (setScreen) setScreen("home"); }}
                style={{ background:`${t.danger}18`, border:`2px solid ${t.danger}`, color:t.danger, fontFamily:t.fontDisplay, fontSize:ip?12:17, fontWeight:700, padding:ip?"10px 28px":"14px 52px", borderRadius:ip?2:10, cursor:"pointer", letterSpacing:"0.08em" }}
                onMouseEnter={e => { playHover?.(); e.currentTarget.style.background=t.danger; e.currentTarget.style.color="#000"; e.currentTarget.style.boxShadow=`0 6px 28px ${t.danger}55`; }}
                onMouseLeave={e => { e.currentTarget.style.background=`${t.danger}18`; e.currentTarget.style.color=t.danger; e.currentTarget.style.boxShadow="none"; }}
              >YES, FORFEIT</button>
              <button className="action-btn"
                onClick={() => { playClick?.(); pausedRef.current = false; setShowSurrender(false); }}
                style={{ background:`${t.accent}18`, border:`2px solid ${t.accent}`, color:t.accent, fontFamily:t.fontDisplay, fontSize:ip?12:17, fontWeight:700, padding:ip?"10px 28px":"14px 52px", borderRadius:ip?2:10, cursor:"pointer", letterSpacing:"0.08em" }}
                onMouseEnter={e => { playHover?.(); e.currentTarget.style.background=t.accent; e.currentTarget.style.color="#000"; e.currentTarget.style.boxShadow=`0 6px 28px ${t.accent}55`; }}
                onMouseLeave={e => { e.currentTarget.style.background=`${t.accent}18`; e.currentTarget.style.color=t.accent; e.currentTarget.style.boxShadow="none"; }}
              >NO, STAY</button>
            </div>
          </div>
        </div>
      ...
      {showExitConfirm && (
        <div className="overlay-backdrop" style={{ position:"fixed", inset:0, zIndex:9999, background:"rgba(0,0,0,0.88)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:28 }}>
          <div className="overlay-modal" style={{ background:t.bgPanel, border:`${ip?3:1}px solid ${t.border}`, borderRadius:ip?2:20, padding:ip?"32px 36px":"48px 56px", maxWidth:520, width:"90vw", textAlign:"center", boxShadow:`0 40px 100px rgba(0,0,0,0.7)` }}>
            ...
            <div style={{ fontFamily:t.fontDisplay, fontSize:ip?14:23, fontWeight:700, color:t.text, lineHeight:1.5, marginBottom:12 }}>
            ...
            <div style={{ display:"flex", gap:16, justifyContent:"center" }}>
              <button className="action-btn"
                onClick={() => { setShowExitConfirm(false); if (setScreen) setScreen("home"); }}
                style={{ background:`${t.danger}18`, border:`2px solid ${t.danger}`, color:t.danger, fontFamily:t.fontDisplay, fontSize:ip?12:17, fontWeight:700, padding:ip?"10px 28px":"14px 52px", borderRadius:ip?2:10, cursor:"pointer", letterSpacing:"0.08em" }}
                onMouseEnter={e => { playHover?.(); e.currentTarget.style.background=t.danger; e.currentTarget.style.color="#000"; e.currentTarget.style.boxShadow=`0 6px 28px ${t.danger}55`; }}
                onMouseLeave={e => { e.currentTarget.style.background=`${t.danger}18`; e.currentTarget.style.color=t.danger; e.currentTarget.style.boxShadow="none"; }}
              >YES</button>
              <button className="action-btn"
                onClick={() => { playClick?.(); pausedRef.current = false; setShowExitConfirm(false); }}
                style={{ background:`${t.accent}18`, border:`2px solid ${t.accent}`, color:t.accent, fontFamily:t.fontDisplay, fontSize:ip?12:17, fontWeight:700, padding:ip?"10px 28px":"14px 52px", borderRadius:ip?2:10, cursor:"pointer", letterSpacing:"0.08em" }}
                onMouseEnter={e => { playHover?.(); e.currentTarget.style.background=t.accent; e.currentTarget.style.color="#000"; e.currentTarget.style.boxShadow=`0 6px 28px ${t.accent}55`; }}
                onMouseLeave={e => { e.currentTarget.style.background=`${t.accent}18`; e.currentTarget.style.color=t.accent; e.currentTarget.style.boxShadow="none"; }}
              >NO</button>
      ...
      <style>{`
        ...
        @keyframes redWinCellPulse {
          0%,100% { box-shadow: 0 0 10px rgba(255,80,0,0.3); }
          50%     { box-shadow: 0 0 28px rgba(255,80,0,0.7), inset 0 0 16px rgba(255,40,0,0.2); }
        }
        ...
        @keyframes winCellPulse {
          0%,100% { background: color-mix(in srgb, var(--win-col) 28%, transparent); box-shadow: 0 0 8px color-mix(in srgb, var(--win-col) 44%, transparent); }
          50%     { background: color-mix(in srgb, var(--win-col) 60%, transparent); box-shadow: 0 0 22px color-mix(in srgb, var(--win-col) 80%, transparent); }
        }
        ...
        .action-btn {
          transition: background 0.25s cubic-bezier(.22,.68,0,1.2),
                      color      0.25s cubic-bezier(.22,.68,0,1.2),
                      transform  0.2s  cubic-bezier(.22,.68,0,1.2),
                      box-shadow 0.25s cubic-bezier(.22,.68,0,1.2) !important;
        }
...
@keyframes iceWinCellPulse {
  0%,100% { box-shadow: 0 0 10px rgba(100,200,255,0.3); }
  50%     { box-shadow: 0 0 28px rgba(100,200,255,0.7), inset 0 0 16px rgba(60,160,255,0.2); }
}
      `}</style>
    </div>
  );
