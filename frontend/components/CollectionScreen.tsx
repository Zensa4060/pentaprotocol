...
const BOARD_BUNDLES = [
  ...
  },
  {
    ...
    accentColor: "#FF4400", isDefault: false, isFlameSkull: true, price: 2198,
  ...
  },
  {
    ...
    accentColor: "#7dd3fc", isDefault: false, isSnowShard: true, price: 2198,
  ...
  },
  {
    ...
    accentColor: "#a5f3fc", isDefault: false, isGlacierShard: true, price: 2198,
  ...
  },
  {
    ...
    accentColor: "#dc2626", isDefault: false, price: 2198,
  ...
  },
  {
    ...
    accentColor: "#F59E0B", isDefault: false, price: 2198,
  ...
  },
  {
    ...
    accentColor: "#ff00b4", isDefault: false, price: 2198,
  ...
  },
  {
    ...
    accentColor: "#00ff41", isDefault: false, price: 2198,
  ...
  },
  {
    ...
    accentColor: "#a855f7", isDefault: false, price: 2198,
  ...
  },
  {
    ...
    accentColor: "#00ffd0", isDefault: false, price: 2198,
  ...
  },
  {
    ...
    accentColor: "#ff6600", isDefault: false, price: 2198,
  ...
  },
  {
    ...
    accentColor: "#8b5cf6", isDefault: false, price: 2198,
  ...
  },
  {
    ...
    accentColor: "#00d9ff", isDefault: false, price: 2198,
  ...
  },
  {
    ...
    accentColor: "#ffdd00", isDefault: false, price: 2198,
  ...
  },
  {
    ...
    accentColor: "#ff0066", isDefault: false, price: 2198,
...
export default function CollectionScreen({ themeId, setThemeIdAction, onHoverAction, onClickAction, initialTab }: Props) {
  ...
  const cardBg = isLight ? "rgba(242,245,250,0.97)" : "rgba(30,30,30,0.6)";
  const cardBorderOwned = isLight ? "rgba(15,22,35,0.12)" : "rgba(255,255,255,0.1)";
  const cardBorderLocked = isLight ? "rgba(15,22,35,0.07)" : "rgba(255,255,255,0.05)";
  const lockChipBg = isLight ? "rgba(226,231,240,0.96)" : "rgba(0,0,0,0.8)";
  const lockChipBorder = isLight ? "rgba(15,22,35,0.16)" : "rgba(255,255,255,0.15)";
  const lockChipText = isLight ? "#5C7088" : "#888";
  const lockIconColor = isLight ? "#5C7088" : "#888";
  const lockIconColorDeep = isLight ? "#8090A8" : "#555";
  const coinPlaceholderBg = isLight ? "#C0CAD8" : "#1a1a1a";
  const coinPlaceholderIcon = isLight ? "#8090A8" : "#555";
  const tossUnownedRingStroke = isLight ? "#8090A8" : "#333";
  const dividerColor = isLight ? "rgba(15,22,35,0.10)" : "rgba(255,255,255,0.1)";
  ...
  const confirmPurchase = async () => {
    ...
    try {
      await API.post(
        "/api/store/purchase-item",
        { item_id: purchaseModal.id, price: purchaseModal.price, shard_price: 0 },
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 2, background: themeId === "space" ? "transparent" : t.bg, paddingTop: 84, overflowY: "auto", transition: "background 0.4s" }}>
              ...
              {BOARD_BUNDLES.filter(b => showAll || b.bOwned(profile) || b.pOwned(profile) || b.isDefault).map((bundle, i) => {
                ...
                return (
                  <div key={`${bundle.id}-${i}`}
                    ...
                    style={{ minWidth: isMobile ? 260 : 320, maxWidth: isMobile ? 260 : 320, flex: `0 0 ${isMobile ? 260 : 320}px`, scrollSnapAlign: "start", borderRadius: 18, overflow: "hidden", border: `1.5px solid ${anyOwned ? (isActive ? ac : (isLight ? "rgba(0,0,0,0.1)" : "rgba(255,255,255,0.1)")) : (isLight ? "rgba(0,0,0,0.06)" : "rgba(255,255,255,0.04)")}`, background: isLight ? cardBg : "rgba(18,18,26,0.9)", backdropFilter: "blur(14px)", boxShadow: isActive ? `0 0 28px ${ac}28` : (isLight ? "0 2px 12px rgba(0,0,0,0.08)" : "none"), cursor: anyOwned && !isActive ? "pointer" : "default", position: "relative", transition: "box-shadow 0.2s, border-color 0.2s" }}>
                    ...
                    <div style={{ padding: "14px 16px" }}>
                      ...
                      <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
                        {/* P1 */}
                        <div style={{ flex: 1, background: pOk || bundle.isDefault ? `${ac}0D` : (isLight ? "rgba(0,0,0,0.04)" : "#0c0c14"), border: `1px solid ${pOk || bundle.isDefault ? `${ac}2A` : (isLight ? "rgba(0,0,0,0.07)" : "rgba(255,255,255,0.05)")}`, borderRadius: 8, padding: "8px 4px", display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
                          <div style={{ width: 32, height: 32, position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}>
                            {(bundle as any).isFlameSkull ? (<svg width="22" height="22" viewBox="0 0 100 120" fill="none"><path d="M50 10 C30 30 15 50 25 70 C20 60 35 55 30 70 C25 85 35 100 50 110 C65 100 75 85 70 70 C65 55 80 60 75 70 C85 50 70 30 50 10Z" fill={pOk ? "#FF4400" : (isLight ? "#BBBBBB" : "#333")} opacity={pOk ? 0.9 : 0.3}/></svg>)
                            : (bundle as any).isGlacierShard ? (pOk ? <GlacierSigilPiece cssSize="26px" /> : <span style={{ fontSize: 14, color: isLight ? "#AAAAAA" : "#333" }}>✶</span>)
                            : <span style={{ fontFamily: t.fontMono, fontSize: 16, fontWeight: 900, color: pOk || bundle.isDefault ? bundle.pieceP1c : (isLight ? "#AAAAAA" : "#333") }}>{bundle.pieceP1}</span>}
                          </div>
                          <span style={{ fontFamily: t.fontMono, fontSize: 7, color: pOk || bundle.isDefault ? `${ac}77` : (isLight ? "#AAAAAA" : "#333"), letterSpacing: "0.1em", fontWeight: 700 }}>P1</span>
                        </div>
                        {/* P2 */}
                        <div style={{ flex: 1, background: pOk || bundle.isDefault ? `${ac}0D` : (isLight ? "rgba(0,0,0,0.04)" : "#0c0c14"), border: `1px solid ${pOk || bundle.isDefault ? `${ac}2A` : (isLight ? "rgba(0,0,0,0.07)" : "rgba(255,255,255,0.05)")}`, borderRadius: 8, padding: "8px 4px", display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
                          <div style={{ width: 32, height: 32, position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}>
                            {(bundle as any).isFlameSkull ? (<svg width="18" height="18" viewBox="0 0 100 110" fill="none" stroke={pOk ? "#CCCCCC" : (isLight ? "#BBBBBB" : "#333")} strokeWidth="6" strokeLinecap="round" strokeLinejoin="round" opacity={pOk ? 1 : 0.3}><path d="M20 65 C20 35 80 35 80 65 C80 80 72 88 72 95 L28 95 C28 88 20 80 20 65Z"/><circle cx="37" cy="62" r="10" fill={pOk ? "#EEE" : (isLight ? "#BBBBBB" : "#333")}/><circle cx="63" cy="62" r="10" fill={pOk ? "#EEE" : (isLight ? "#BBBBBB" : "#333")}/></svg>)
                            : (bundle as any).isGlacierShard ? (pOk ? <GlacierPrismPiece cssSize="26px" /> : <span style={{ fontSize: 14, color: isLight ? "#AAAAAA" : "#333" }}>◈</span>)
                            : <span style={{ fontFamily: t.fontMono, fontSize: 16, fontWeight: 900, color: pOk || bundle.isDefault ? bundle.pieceP2c : (isLight ? "#AAAAAA" : "#333") }}>{bundle.pieceP2}</span>}
                          </div>
                          <span style={{ fontFamily: t.fontMono, fontSize: 7, color: pOk || bundle.isDefault ? `${ac}77` : (isLight ? "#AAAAAA" : "#333"), letterSpacing: "0.1em", fontWeight: 700 }}>P2</span>
                        </div>
                        {/* Board mini */}
                        <div style={{ flex: 1.4, background: bOk || bundle.isDefault ? bundle.boardPreview : (isLight ? "rgba(0,0,0,0.04)" : "#0c0c14"), border: `1px solid ${bOk || bundle.isDefault ? bundle.boardBorder + "55" : (isLight ? "rgba(0,0,0,0.07)" : "rgba(255,255,255,0.05)")}`, borderRadius: 8, padding: "8px 4px", display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
                    ...
                    <div style={{ height: 132, overflow: "hidden", position: "relative" }}>
                      ...
                      {owned && isActive && (
                        <>
                          ...
                          <div style={{ position: "absolute", inset: 0, background: "linear-gradient(110deg, transparent 40%, rgba(255,255,255,0.08) 45%, rgba(255,255,255,0.14) 50%, rgba(255,255,255,0.08) 55%, transparent 60%)", backgroundSize: "200% 100%", animation: "bannerShine 3s infinite linear", zIndex: 2, pointerEvents: "none" }} />
                        </>
                      )}
                    </div>
                    <div style={{ padding: "16px" }}>
