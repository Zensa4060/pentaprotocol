...
function draw(ctx: CanvasRenderingContext2D, t: number, W: number, H: number, s: any) {
  ...
  s.spray.forEach((sp: any)=>{if(sp.life<=0)return;sp.x+=sp.vx;sp.y+=sp.vy;sp.vy+=0.07;sp.life-=1/30;const a=sp.life*0.8;ctx.beginPath();ctx.arc(sp.x,sp.y,sp.r,0,Math.PI*2);ctx.fillStyle=`rgba(180,225,255,${a})`;ctx.fill();});
