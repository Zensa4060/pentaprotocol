...
export default function StoreScreen({ setScreen, themeId }: Props) {
  ...
  const cssVars = {
    ...
    "--text": t.text, "--text-muted": t.textMuted, "--border": t.border, "--accent": accent,
...
for (let i = 0; i < 3; i++) {
    ...
    await new Promise(r => setTimeout(r, 2000));
      ...
      await new Promise<void>((resolve, reject) => {
        const rz = new window.Razorpay({
          ...
          prefill: { name: user!.username, email: (user as any).email || "" },
          theme: { color: accent },
          ...
          handler: async (response: any) => {
            try {
              ...
              updateUser({ protocredits: ((user as any)?.protocredits ?? 0) + verifyRes.data.credits_added });
              resolve();
            } catch (e) { reject(e); }
          },
        });
  ...
  // Button shown to guests instead of a price button
  const GuestBuyBtn = () => (
    <button
      onClick={() => setMsg({ text: "Sign in to purchase premium equipment.", ok: false })}
      style={{ flexShrink: 0, background: `${t.textMuted}14`, border: `1.5px solid ${t.border}`, borderRadius: 8, padding: "6px 11px", fontFamily: t.fontMono, fontSize: 10, fontWeight: 700, color: t.textMuted, cursor: "pointer", whiteSpace: "nowrap" as const, display: "flex", alignItems: "center", gap: 4 }}>
      🔒 Sign in
    </button>
  );
    ...
    <div style={{ ...cssVars, minHeight: "100vh", background: t.bg, transition: "background 0.4s", paddingTop: 84, overflowY: "auto" }}>
      ...
      <div style={{ maxWidth: 1240, margin: "0 auto", padding: "0 28px 72px" }}>
        ...
        {isGuest && (
          <div style={{ background: `${accent}10`, border: `1px solid ${accent}44`, borderRadius: 10, padding: "12px 18px", marginBottom: 28, display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <button onClick={() => setScreen("auth")}
              style={{ marginLeft: "auto", flexShrink: 0, background: accent, border: "none", color: "#000", fontFamily: t.fontDisplay, fontSize: 12, fontWeight: 800, padding: "7px 16px", borderRadius: 7, cursor: "pointer", letterSpacing: "0.06em" }}>
              SIGN IN
            </button>
          </div>
        )}
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 48, gap: 24, flexWrap: "wrap" as const }}>
          ...
          {/* ProtoCredits buy panel — clicking blocked for guests */}
          <div className="store-card"
            onClick={() => { if (isGuest) { setMsg({ text: "Sign in to buy ProtoCredits.", ok: false }); return; } setShowBuyModal(true); }}
                    ...
                    {item.isFlameSkull ? (
                      <>
                        <div style={{ width: 52, height: 52, borderRadius: 8, background: "rgba(255,68,0,0.12)", border: "2px solid #FF4400", display: "flex", alignItems: "center", justifyContent: "center" }}>
                          <svg width="30" height="30" viewBox="0 0 100 120" fill="none"><path d="M50 10 C30 30 15 50 25 70 C20 60 35 55 30 70 C25 85 35 100 50 110 C65 100 75 85 70 70 C65 55 80 60 75 70 C85 50 70 30 50 10Z" fill="#FF4400" opacity="0.9"/><path d="M50 40 C40 55 38 65 45 75 C43 68 50 65 48 75 C46 85 50 95 50 100 C55 90 58 80 55 70 C53 62 60 58 58 68 C65 55 58 42 50 40Z" fill="#FFB300" opacity="0.85"/></svg>
                        </div>
                        <div style={{ width: 52, height: 52, borderRadius: 8, background: "rgba(170,170,170,0.08)", border: "2px solid #AAAAAA", display: "flex", alignItems: "center", justifyContent: "center" }}>
                          <svg width="30" height="30" viewBox="0 0 100 110" fill="none" stroke="#CCCCCC" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 65 C20 35 80 35 80 65 C80 80 72 88 72 95 L28 95 C28 88 20 80 20 65Z"/><rect x="30" y="95" width="40" height="10" rx="3"/><circle cx="37" cy="62" r="9" fill="#CCCCCC"/><circle cx="63" cy="62" r="9" fill="#CCCCCC"/><line x1="50" y1="95" x2="50" y2="105"/><line x1="37" y1="95" x2="37" y2="105"/><line x1="63" y1="95" x2="63" y2="105"/></svg>
                        </div>
                      </>
                    ) : (
                      <>
