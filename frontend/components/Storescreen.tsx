...
export default function StoreScreen({ setScreenAction, themeId }: Props) {
  ...
  const proceedBuyCosmetic = async () => {
    ...
    try {
      ...
      if (bundleData) {
        ...
        const shardTarget = themeBundleData.shardPrice ?? shardPrice;
  ...
  return (
    <div style={{ ...cssVars, minHeight: "100vh", background: themeId === "pixel" ? "url(/bg-pixel.png) center/cover no-repeat" : themeId === "space" ? "transparent" : t.bg, transition: "background 0.4s", paddingTop: 84, overflowY: "auto" }}>
      <style>{`
          ...
          <div style={{ paddingTop: 14, paddingInline: 10 }}>
            <InfiniteCarouselRow
              ...
              (() => {
                ...
                const shardPrice = item.shardPrice ?? 0;
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                        {owned ? (
                          ...
                          <div style={{ fontFamily: t.fontMono, fontSize: 12, fontWeight: 900, color: glow, letterSpacing: "0.06em", display: "flex", alignItems: "center", gap: 6 }}>
                            {price.toLocaleString()} <ProtoSVG size={16} /> {shardPrice.toLocaleString()} <ShardSVG size={16} />
                          </div>
                        )}
