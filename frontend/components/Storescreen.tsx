...
export default function StoreScreen({ setScreenAction, themeId, audio, initialSection, initialPreview }: Props) {
  ...
  //   syros_skin  ← granted on FIRST SYROS defeat in the unranked queue
  ...
  const canClaimFreeSyrosSkin= botRewards.syros_skin === "pending";
  // Combined eligibility — drives the BundleCard's "claim free" affordance.
  // Either SYROS or HER pending is enough to enable the per-bundle button;
  // the click handler below picks which slot to consume (SYROS first so
  ...
  const claimFreeReward = async (
    ...
    const url =
      ...
      slot === "syros_skin" ? "/api/profile/claim-syros-board-skin-reward" :
    ...
    const successText =
      ...
      slot === "syros_skin" ? `✓ ${label} board skin unlocked — SYROS reward claimed!` :
  ...
  // SYROS reward are pending we prefer to consume the SYROS slot first
  ...
  const claimFreeBoardSkin = (boardSkinId: string, label: string) => {
    const slot: "syros_skin" | "board_skin" =
      canClaimFreeSyrosSkin ? "syros_skin" : "board_skin";
      ...
      <div style={{ maxWidth: 1240, margin: "0 auto", padding: "0 28px 72px" }}>
        ...
        <div style={{ marginBottom: 56 }}>
          ...
          {canClaimFreeSyrosSkin && (
             ...
             * used for both rewards; the click handler picks the SYROS
          ...
          {canClaimFreeBoardSkin && (
            <div style={{
              ...
              <div style={{ fontFamily: t.fontMono, fontSize: 11, color: "#FCD34D", letterSpacing: "0.14em", padding: "6px 10px", border: "1px solid #FCD34D88", borderRadius: 8, whiteSpace: "nowrap" as const }}>
                {canClaimFreeSyrosSkin ? "2 REWARDS REMAINING" : "1 REWARD REMAINING"}
              </div>
            </div>
          )}
