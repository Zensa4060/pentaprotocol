...
# ═══════════════════════════════════════════════════════════════════════════════
# Uppercase: width W=460, cap-height H=700, stroke SW=80
# Lowercase: width WL=400, x-height XH=490, stroke SW=80
# Digits share uppercase proportions.
# Descenders reach -200.

W=460; H=700; SW=80
MH=(H-SW)//2   # 310  mid-bar lower y
MC=MH+SW       # 390  mid-bar upper y
TS=H-SW        # 620  top-stroke lower y
ADV=580        # uppercase advance width

# Lowercase measures
XH=490; WL=400
MHL=(XH-SW)//2  # 205  lowercase mid lower
MCL=MHL+SW      # 285  lowercase mid upper
TSL=XH-SW       # 410  lowercase top-stroke lower y

ADV_L=520       # lowercase advance
ADV_N=360       # narrow (i, j, l, punctuation)
ADV_W=700       # wide (m, w, uppercase M/W)

# ─── Uppercase glyphs ─────────────────────────────────────────────────────────
def po_A(p): draw_glyph(p,[R(0,0,SW,H),R(W-SW,0,W,H),R(0,TS,W,H),R(0,MH,W,MC)])
def po_B(p): draw_glyph(p,[R(0,0,SW,H),R(0,TS,W-SW,H),R(0,MH,W-SW,MC),R(0,0,W-SW,SW),R(W-SW,MH,W,H),R(W-SW,0,W,MC)])
def po_C(p): draw_glyph(p,[R(0,0,SW,H),R(0,TS,W,H),R(0,0,W,SW)])
def po_D(p): draw_glyph(p,[R(0,0,SW,H),R(W-SW,SW,W,TS),R(SW,TS,W,H),R(SW,0,W,SW)])
...
def po_G(p): draw_glyph(p,[R(0,0,SW,H),R(0,TS,W,H),R(0,0,W,SW),R(W//2,MC-SW//2,W,MC+SW//2+SW),R(W-SW,0,W,MC+SW)])
def po_H(p): draw_glyph(p,[R(0,0,SW,H),R(W-SW,0,W,H),R(0,MH,W,MC)])
def po_I(p): draw_glyph(p,[R(W//2-SW//2,0,W//2+SW//2,H),R(0,TS,W,H),R(0,0,W,SW)])
def po_J(p): draw_glyph(p,[R(0,TS,W,H),R(W-SW,SW*2,W,TS),R(0,0,W,SW),R(0,SW,SW*2,SW*3)])
def po_K(p): draw_glyph(p,[R(0,0,SW,H),R(0,MH,W//2+SW//2,MC),D(W//2,MC,W,H,SW),D(W//2,MH,W,0,SW)])
def po_L(p): draw_glyph(p,[R(0,0,SW,H),R(0,0,W,SW)])
def po_M(p): draw_glyph(p,[R(0,0,SW,H),R(W-SW,0,W,H),R(0,TS,W,H),D(SW,H,W//2+SW//4,H//3,int(SW*1.1)),D(W//2-SW//4,H//3,W-SW,H,int(SW*1.1))])
def po_N(p): draw_glyph(p,[R(0,0,SW,H),R(W-SW,0,W,H),D(SW//2,H-SW//4,W-SW//2,SW//4,SW)])
...
def po_Q(p): draw_glyph(p,[R(0,0,SW,H),R(W-SW,0,W,H),R(0,TS,W,H),R(0,0,W,SW),R(W//2,0,W,SW*2)])
def po_R(p): draw_glyph(p,[R(0,0,SW,H),R(0,TS,W,H),R(0,MH,W,MC),R(W-SW,MH,W,H),D(W//2,MH,W-SW//4,SW//4,SW)])
def po_S(p): draw_glyph(p,[R(0,MH,W,MC),R(0,TS,W,H),R(0,0,W,SW),R(0,MH,SW,H),R(W-SW,0,W,MC)])
def po_T(p): draw_glyph(p,[R(0,TS,W,H),R(W//2-SW//2,0,W//2+SW//2,H)])
def po_U(p): draw_glyph(p,[R(0,0,SW,H),R(W-SW,0,W,H),R(0,0,W,SW)])
def po_V(p): draw_glyph(p,[D(0,H,W//2,0,int(SW*1.05)),D(W//2,0,W,H,int(SW*1.05))])
def po_W(p): draw_glyph(p,[D(0,H,W//4,0,SW),D(W//4,0,W//2,H//2,SW),D(W//2,H//2,W*3//4,0,SW),D(W*3//4,0,W,H,SW)])
def po_X(p): draw_glyph(p,[D(0,0,W,H,SW),D(0,H,W,0,SW)])
def po_Y(p): draw_glyph(p,[D(0,H,W//2,H//2,SW),D(W//2,H//2,W,H,SW),R(W//2-SW//2,0,W//2+SW//2,H//2+SW//2)])
def po_Z(p): draw_glyph(p,[R(0,TS,W,H),R(0,0,W,SW),D(W-SW//4,TS+SW//4,SW//4,SW//4,int(SW*1.1))])
...
# ─── Lowercase glyphs ─────────────────────────────────────────────────────────
def po_a(p): draw_glyph(p,[R(0,0,WL,SW),R(0,TSL,WL,XH),R(0,MHL,WL,MCL),R(WL-SW,0,WL,XH)])
def po_b(p): draw_glyph(p,[R(0,0,SW,H),R(0,TSL,WL-SW,XH),R(0,MHL,WL-SW,MCL),R(0,0,WL-SW,SW),R(WL-SW,MHL,WL,XH),R(WL-SW,0,WL,MCL)])
def po_c(p): draw_glyph(p,[R(0,0,SW,XH),R(0,TSL,WL,XH),R(0,0,WL,SW)])
def po_d(p): draw_glyph(p,[R(0,0,SW,XH),R(WL-SW,0,WL,H),R(SW,TSL,WL,XH),R(SW,0,WL,SW)])
def po_e(p): draw_glyph(p,[R(0,0,SW,XH),R(0,TSL,WL,XH),R(0,MHL,WL,MCL),R(0,0,WL,SW)])
def po_f(p): draw_glyph(p,[R(SW,0,SW*2,H),R(SW,int(H*0.75),int(W*0.65),int(H*0.75)+SW),R(0,int(H*0.62),int(W*0.65),int(H*0.62)+SW)])
def po_g(p): draw_glyph(p,[R(0,-SW*2,WL-SW,SW),R(WL-SW,-SW*3,WL,XH),R(SW,TSL,WL,XH),R(SW,0,WL,SW),R(0,0,SW,XH),R(0,-SW*3,WL,SW-SW*3)])
def po_h(p): draw_glyph(p,[R(0,0,SW,H),R(0,MHL,WL,MCL),R(WL-SW,0,WL,MCL)])
def po_i(p):
    cw=SW*3
    draw_glyph(p,[R(0,0,cw,XH),R(0,int(XH*1.15),cw,int(XH*1.15)+SW*2)])
def po_j(p):
    cw=SW*3
    draw_glyph(p,[R(0,-SW*2,cw,XH),R(0,-SW*3,cw+SW,SW-SW*3),R(0,int(XH*1.15),cw,int(XH*1.15)+SW*2)])
def po_k(p): draw_glyph(p,[R(0,0,SW,H),R(0,MHL,WL//2+SW//2,MCL),D(WL//2,MCL,WL,XH,SW),D(WL//2,MHL,WL,0,SW)])
def po_l(p):
    draw_glyph(p,[R(SW//2,0,SW+SW//2,H),R(0,0,SW*3,SW)])
def po_m(p):
    Wm=int(W*1.1)
    draw_glyph(p,[R(0,0,SW,XH),R(Wm-SW,0,Wm,XH),R(Wm//2-SW//2,0,Wm//2+SW//2,XH),
                  R(0,TSL,Wm,XH),R(0,MHL,Wm,MCL)])
...
def po_p(p): draw_glyph(p,[R(0,-SW*3,SW,XH),R(0,TSL,WL-SW,XH),R(0,MHL,WL-SW,MCL),R(0,0,WL-SW,SW),R(WL-SW,MHL,WL,XH),R(WL-SW,0,WL,MCL)])
def po_q(p): draw_glyph(p,[R(WL-SW,-SW*3,WL,XH),R(SW,TSL,WL,XH),R(SW,MHL,WL,MCL),R(SW,0,WL,SW),R(0,MHL,SW,XH),R(0,0,SW,MCL)])
def po_r(p): draw_glyph(p,[R(0,0,SW,XH),R(0,TSL,WL,XH),R(WL-SW,TSL-SW,WL,XH)])
def po_s(p): draw_glyph(p,[R(0,MHL,WL,MCL),R(0,TSL,WL,XH),R(0,0,WL,SW),R(0,MHL,SW,XH),R(WL-SW,0,WL,MCL)])
def po_t(p):
    draw_glyph(p,[R(SW,0,SW*2,H),R(0,int(H*0.62),int(WL*0.8),int(H*0.62)+SW)])
def po_u(p): draw_glyph(p,[R(0,0,SW,XH),R(WL-SW,0,WL,XH),R(0,0,WL,SW)])
def po_v(p): draw_glyph(p,[D(0,XH,WL//2,0,int(SW*1.05)),D(WL//2,0,WL,XH,int(SW*1.05))])
def po_w(p):
    Ww=int(W*1.05)
    draw_glyph(p,[D(0,XH,Ww//4,0,SW),D(Ww//4,0,Ww//2,XH//2,SW),D(Ww//2,XH//2,Ww*3//4,0,SW),D(Ww*3//4,0,Ww,XH,SW)])
def po_x(p): draw_glyph(p,[D(0,0,WL,XH,SW),D(0,XH,WL,0,SW)])
def po_y(p): draw_glyph(p,[D(0,XH,WL//2,XH//3,SW),D(WL//2,XH//3,WL,XH,SW),R(WL//2-SW//2,-SW*3,WL//2+SW//2,XH//3+SW//2)])
def po_z(p): draw_glyph(p,[R(0,TSL,WL,XH),R(0,0,WL,SW),D(WL-SW//4,TSL+SW//4,SW//4,SW//4,int(SW*1.05))])
...
# ─── Digits ───────────────────────────────────────────────────────────────────
def po_0(p): draw_glyph(p,[R(0,0,SW,H),R(W-SW,0,W,H),R(0,TS,W,H),R(0,0,W,SW),D(SW//2,SW//2,W-SW//2,H-SW//2,SW)])
def po_1(p): draw_glyph(p,[R(W//2-SW//2,0,W//2+SW//2,H),R(W//4,TS,W//2,H),R(0,0,W*3//4,SW)])
def po_2(p): draw_glyph(p,[R(0,TS,W,H),R(W-SW,MH,W,H),R(0,MH,W,MC),R(0,0,SW,MC),R(0,0,W,SW)])
def po_3(p): draw_glyph(p,[R(0,TS,W,H),R(0,MH,W,MC),R(0,0,W,SW),R(W-SW,0,W,H)])
def po_4(p): draw_glyph(p,[R(0,MH,W,MC),R(W-SW,0,W,H),R(0,MH,SW,H)])
def po_5(p): draw_glyph(p,[R(0,TS,W,H),R(0,MH,W,MC),R(0,0,W,SW),R(0,MH,SW,H),R(W-SW,0,W,MC)])
def po_6(p): draw_glyph(p,[R(0,0,SW,H),R(0,TS,W,H),R(0,MH,W,MC),R(0,0,W,SW),R(W-SW,0,W,MC)])
def po_7(p): draw_glyph(p,[R(0,TS,W,H),R(W-SW,MH,W,H),R(0,MH,W,MC)])
def po_8(p): draw_glyph(p,[R(0,0,SW,H),R(W-SW,0,W,H),R(0,TS,W,H),R(0,MH,W,MC),R(0,0,W,SW)])
def po_9(p): draw_glyph(p,[R(0,MH,SW,H),R(W-SW,0,W,H),R(0,TS,W,H),R(0,MH,W,MC),R(0,0,W,SW)])
...
def po_exclam(p): draw_glyph(p,[R(W//2-HS,SW*2,W//2+HS,H),R(W//2-HS,0,W//2+HS,SW*1+SW//2)])
def po_quotedbl(p): draw_glyph(p,[R(W//4-HS,TS,W//4+HS,H),R(W*3//4-HS,TS,W*3//4+HS,H)])
def po_numbersign(p): draw_glyph(p,[R(W//4-HS,0,W//4+HS,H),R(W*3//4-HS,0,W*3//4+HS,H),R(0,MH+SW//3,W,MH+SW//3+SW),R(0,MH-SW//3-SW,W,MH-SW//3)])
def po_dollar(p):
    draw_glyph(p,[R(0,MH,W,MC),R(0,TS,W,H),R(0,0,W,SW),R(0,MH,SW,H),R(W-SW,0,W,MC),R(W//2-HS,0-SW,W//2+HS,H+SW)])
def po_percent(p): draw_glyph(p,[D(0,0,W,H,SW),R(0,int(H*0.6),int(W*0.3),H),R(int(W*0.7),0,W,int(H*0.4))])
def po_ampersand(p): draw_glyph(p,[R(0,0,SW,H),R(0,MH,W*3//4,MC),R(0,TS,W*3//4,H),R(0,0,W*2//3,SW),R(W//3,MH,W,SW*2),D(W//3,MH,W,0,SW)])
def po_quotesingle(p): draw_glyph(p,[R(W//2-HS,TS,W//2+HS,H)])
def po_parenleft(p): draw_glyph(p,[R(W*2//5,0,W*2//5+SW,H),R(W//5,SW,W*2//5+SW,SW*2),R(W//5,H-SW*2,W*2//5+SW,H)])
def po_parenright(p): draw_glyph(p,[R(W//5,0,W//5+SW,H),R(W//5,SW,W*3//5,SW*2),R(W//5,H-SW*2,W*3//5,H)])
def po_asterisk(p): draw_glyph(p,[R(W//2-HS,MH-SW,W//2+HS,MC+SW),R(0,MH,W,MC),D(W//6,MH-SW,W*5//6,MC+SW,SW),D(W//6,MC+SW,W*5//6,MH-SW,SW)])
def po_plus(p): draw_glyph(p,[R(0,MH,W,MC),R(W//2-HS,SW*2,W//2+HS,H-SW*2)])
def po_comma(p): draw_glyph(p,[R(W//2-HS,0,W//2+HS,SW*2),R(W//2-HS-QS,SW//2,W//2+HS-QS,SW*2)])
...
def po_semicolon(p): draw_glyph(p,[R(W//2-HS,SW//2,W//2+HS,SW*2),R(W//2-HS-QS,0,W//2+HS-QS,SW),R(W//2-HS,MH,W//2+HS,MC)])
def po_less(p): draw_glyph(p,[D(W,H,SW,H//2,SW),D(SW,H//2,W,0,SW)])
def po_equal(p): draw_glyph(p,[R(W//8,MH-SW//2,W*7//8,MH+SW//2),R(W//8,MC-SW//2,W*7//8,MC+SW//2)])
def po_greater(p): draw_glyph(p,[D(0,H,W-SW,H//2,SW),D(W-SW,H//2,0,0,SW)])
def po_question(p): draw_glyph(p,[R(0,TS,W,H),R(W-SW,MH,W,H),R(0,MH,W,MC),R(W//2-HS,SW*2,W//2+HS,MC),R(W//2-HS,0,W//2+HS,SW)])
def po_at(p): draw_glyph(p,[R(0,0,SW,H),R(W-SW,SW*2,W,H),R(0,TS,W,H),R(0,0,W-SW,SW),R(W//2,SW*3,W-SW,H-SW*3),R(W//2,MHL,W-SW,MCL)])
def po_bracketleft(p): draw_glyph(p,[R(W//4,0,W//4+SW,H),R(W//4,TS,W*3//4,H),R(W//4,0,W*3//4,SW)])
def po_backslash(p): draw_glyph(p,[D(0,H,W,0,SW)])
def po_bracketright(p): draw_glyph(p,[R(W*3//4-SW,0,W*3//4,H),R(W//4,TS,W*3//4,H),R(W//4,0,W*3//4,SW)])
def po_asciicircum(p): draw_glyph(p,[D(0,H//2,W//2,H,SW),D(W//2,H,W,H//2,SW)])
def po_underscore(p): draw_glyph(p,[R(0,-SW//2,W,SW//2)])
def po_grave(p): draw_glyph(p,[D(W//4,H,W*3//4,TS,SW)])
def po_braceleft(p): draw_glyph(p,[R(W//3,0,W//3+SW,H),R(W//3,TS,W*3//4,H),R(W//3,0,W*3//4,SW),R(W//3,MH,W,MC),R(W//6,MH,W//3+SW,MC)])
def po_bar(p): draw_glyph(p,[R(W//2-HS,0,W//2+HS,H)])
def po_braceright(p): draw_glyph(p,[R(W*2//3-SW,0,W*2//3,H),R(W//4,TS,W*2//3,H),R(W//4,0,W*2//3,SW),R(0,MH,W*2//3,MC),R(W*2//3-SW,MH,W*5//6,MC)])
def po_asciitilde(p): draw_glyph(p,[D(0,H//2-SW,W//3,H//2+SW,SW),D(W//3,H//2+SW,W*2//3,H//2-SW,SW),D(W*2//3,H//2-SW,W,H//2+SW,SW)])

def po_notdef(p):
    draw_glyph(p,[R(SW,0,W-SW,SW),R(SW,H-SW,W-SW,H),R(SW,0,SW*2,H),R(W-SW*2,0,W-SW,H)])
